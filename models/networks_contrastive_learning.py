...
def init_net_Seg_data_parallel(net, init_type='kaiming', gpu_ids=None, model_parallel=False):
    if gpu_ids is None:
        gpu_ids = []
    ...
    if len(gpu_ids) > 0:
        ...
        net.cuda(gpu_ids[0])
        # 关键修复：仅当 model_parallel=True 时才用DataParallel
        if model_parallel and len(gpu_ids) > 1:
...
def define_registration_model(
    gpu_ids=None,  # 参数1：GPU编号
    is_training=True,  # 参数2：是否训练模式
    model_parallel=False,  # 参数3：是否多GPU（新增传递）
    mode='lpba40',  # 参数4：数据集模式
    img_size=(144, 192, 144)  # 参数5：图像尺寸
):
    ...
    # 关键：传递 model_parallel 参数
    return init_net_Seg_data_parallel(netSeg, init_type='kaiming', gpu_ids=gpu_ids, model_parallel=model_parallel)
